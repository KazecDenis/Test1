...
namespace Watermelon.SquadShooter
{
    public class CharacterBehaviour : MonoBehaviour, IEnemyDetector, IHealth, INavMeshAgent
    {
        ...
        {
            _direction += joystick.FormatInput.normalized;
            if (joystick.IsJoysticTouched && _direction.sqrMagnitude > 0.1f)
            {
                if (!isMoving)
                {
                    isMoving = true;
                ...
                graphics.OnMoving(Mathf.InverseLerp(0, activeMovementSettings.MoveSpeed, speed), _direction, isCloseEnemyFound);

                if (!isCloseEnemyFound)
                {
                    transform.rotation = Quaternion.Lerp(transform.rotation, Quaternion.LookRotation(_direction.normalized), Time.deltaTime * activeMovementSettings.RotationSpeed);
                }
            }
            else
            {
        ...
        private void DesktopMovement()
        {
            ...
            if (_direction.sqrMagnitude > 0.1f)
            {
                ...
                if (!isCloseEnemyFound)
                {
                    transform.rotation = Quaternion.Lerp(transform.rotation, Quaternion.LookRotation(_direction), Time.deltaTime * activeMovementSettings.RotationSpeed);
                }
            }
            else
            {
