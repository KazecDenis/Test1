...
namespace Watermelon.Outline
{
    public static class OutlineEffect
    {
        ...
        public static readonly int ColorHash = Shader.PropertyToID("_OutlineColor");
        ...
        {
            var material = new Material(Resources.Load<Shader>(string.Format("Shaders/{0}", shaderName)));
        ...
        private static void Postprocess(OutlineParameters parameters, int first, int second, Material material, int iterations, bool additionalShift, float shiftValue, ref int stencil, Rect viewport, float scale)
        {
            ...
            for (var index = 1; index <= iterations; index++)
            {
                ...
                Blit(parameters, RTUtility.ComposeTarget(parameters, first), RTUtility.ComposeTarget(parameters, second), RTUtility.ComposeTarget(parameters, first), material, shiftValue, null, -1, viewport);

                stencil = (stencil + 1) % 255;
                parameters.Buffer.SetGlobalInt(RefHash, stencil);

                parameters.Buffer.SetGlobalVector(ShiftHash, new Vector4(0, shift * scale));
                Blit(parameters, RTUtility.ComposeTarget(parameters, second), RTUtility.ComposeTarget(parameters, first), RTUtility.ComposeTarget(parameters, first), material, shiftValue, null, -1, viewport);
        ...
        {
            parameters.Buffer.SetGlobalVector(ScaleHash, (Vector2)parameters.Scale);
            ...
            RTUtility.GetTemporaryRT(parameters, TargetHash, targetWidth, targetHeight, 24, true, false, false);
            ...
            RTUtility.GetTemporaryRT(parameters, PrimaryBufferHash, scaledWidth, scaledHeight, 24, true, false, false);
            RTUtility.GetTemporaryRT(parameters, HelperBufferHash, scaledWidth, scaledHeight, 24, true, false, false);
            ...
            if (parameters.UseInfoBuffer)
            {
                ...
                RTUtility.GetTemporaryRT(parameters, InfoTargetHash, targetWidth, targetHeight, 0, false, false, false);

                RTUtility.GetTemporaryRT(parameters, PrimaryInfoBufferHash, scaledInfoWidth, scaledInfoHeight, 0, true, true, false);
                RTUtility.GetTemporaryRT(parameters, HelperInfoBufferHash, scaledInfoWidth, scaledInfoHeight, 0, true, true, false);
            }
            ...
            parameters.Buffer.SetRenderTarget(RTUtility.ComposeTarget(parameters, TargetHash), RTUtility.ComposeTarget(parameters, parameters.DepthTarget));
            ...
            if (parameters.UseInfoBuffer)
            {
                ...
                parameters.Buffer.SetRenderTarget(RTUtility.ComposeTarget(parameters, InfoTargetHash), parameters.DepthTarget);
                parameters.Buffer.ClearRenderTarget(false, true, Color.clear);
                if (parameters.CustomViewport.HasValue)
                    parameters.Buffer.SetViewport(parameters.CustomViewport.Value);

                DrawOutlineables(parameters, CompareFunction.Always, x => x.DefaultOutlineProperty.Enabled,   x => new Color(x.DefaultOutlineProperty.DilateShift, x.DefaultOutlineProperty.BlurShift, 0, 1),
                    x => OutlineMaterial,
                    RenderStyle.Single, 
                    OutlinableDrawingMode.Normal);

                DrawOutlineables(parameters, CompareFunction.NotEqual, x => x.BackOutlineProperty.Enabled,      x => new Color(x.BackOutlineProperty.DilateShift, x.BackOutlineProperty.BlurShift, 0, 1),
                    x => OutlineMaterial,
                    RenderStyle.FrontBack);

                DrawOutlineables(parameters, CompareFunction.LessEqual, x => x.FrontOutlineProperty.Enabled,     x => new Color(x.FrontOutlineProperty.DilateShift, x.FrontOutlineProperty.BlurShift, 0, 1),
                    x => OutlineMaterial,
                    RenderStyle.FrontBack, 
                    OutlinableDrawingMode.Normal);
                ...
                Blit(parameters,
                    RTUtility.ComposeTarget(parameters, InfoTargetHash),
                    RTUtility.ComposeTarget(parameters, PrimaryInfoBufferHash),
                    RTUtility.ComposeTarget(parameters, PrimaryInfoBufferHash),
                ...
                if (iterationsCount > 5)
                {
                    ...
                    Blit(parameters,
                         RTUtility.ComposeTarget(parameters, InfoTargetHash),
                         RTUtility.ComposeTarget(parameters, HelperInfoBufferHash),
                         RTUtility.ComposeTarget(parameters, HelperInfoBufferHash),
                         BasicBlitMaterial, effectShift, null,
                         -1);
                }
            ...
            parameters.Buffer.SetRenderTarget(RTUtility.ComposeTarget(parameters, TargetHash), parameters.DepthTarget);
            parameters.Buffer.ClearRenderTarget(false, true, Color.clear);
            if (parameters.CustomViewport.HasValue)
                parameters.Buffer.SetViewport(parameters.CustomViewport.Value);

            var drawnOutlinablesCount = 0;
            drawnOutlinablesCount += DrawOutlineables(parameters, CompareFunction.Always, x => x.DefaultOutlineProperty.Enabled, x => x.DefaultOutlineProperty.Color,
                x => OutlineMaterial,
                RenderStyle.Single, OutlinableDrawingMode.Normal);

            drawnOutlinablesCount += DrawOutlineables(parameters, CompareFunction.NotEqual, x => x.BackOutlineProperty.Enabled, x => x.BackOutlineProperty.Color,
                x => OutlineMaterial,
                RenderStyle.FrontBack, OutlinableDrawingMode.Normal);

            drawnOutlinablesCount += DrawOutlineables(parameters, CompareFunction.LessEqual, x => x.FrontOutlineProperty.Enabled, x => x.FrontOutlineProperty.Color,
                x => OutlineMaterial,
                RenderStyle.FrontBack, OutlinableDrawingMode.Normal);
            ...
            if (drawnOutlinablesCount > 0)
            {
                parameters.Buffer.SetGlobalInt(ComparisonHash, (int)CompareFunction.Always);
                parameters.Buffer.SetGlobalInt(OperationHash, (int)StencilOp.Keep);
                Blit(parameters, RTUtility.ComposeTarget(parameters, TargetHash),
                    RTUtility.ComposeTarget(parameters, PrimaryBufferHash),
                    RTUtility.ComposeTarget(parameters, PrimaryBufferHash),
                ...
                if (parameters.BlurIterations + parameters.DilateIterations > 5)
                {
                    ...
                    parameters.Buffer.SetGlobalInt(OperationHash, (int)StencilOp.Keep);
                    Blit(parameters, RTUtility.ComposeTarget(parameters, TargetHash),
                         RTUtility.ComposeTarget(parameters, HelperBufferHash),
                         RTUtility.ComposeTarget(parameters, HelperBufferHash),
                         BasicBlitMaterial, effectShift, null, -1,
                         new Rect(0, 0, scaledViewVector.x, scaledViewVector.y));
                }
        ...
        private static int DrawOutlineables(OutlineParameters parameters, CompareFunction function, Func<Outlinable, bool> shouldRender, Func<Outlinable, Color> colorProvider, Func<Outlinable, Material> materialProvider, RenderStyle styleMask, OutlinableDrawingMode modeMask = OutlinableDrawingMode.Normal)
        {
            ...
            foreach (var targetGroup in targets)
            {
                ...
                parameters.Buffer.DrawRenderer(target.Renderer, materialToUse, target.SubmeshIndex);
            }
        ...
        private static void DrawFill(OutlineParameters parameters, RenderTargetIdentifier targetSurface)
        {
            ...
            foreach (var outlinable in parameters.OutlinablesToRender)
            {
                ...
                foreach (var target in outlinable.OutlineTargets)
                {
                    ...
                    parameters.Buffer.DrawRenderer(renderer, FillMaskMaterial, target.SubmeshIndex);
                }
            }
            ...
            foreach (var outlinable in parameters.OutlinablesToRender)
            {
                ...
                {
                    if ((outlinable.BackOutlineProperty.FillPass.Material == null || !outlinable.BackOutlineProperty.Enabled) &&
                        (outlinable.FrontOutlineProperty.FillPass.Material == null || !outlinable.FrontOutlineProperty.Enabled))
                        continue;

                    var frontMaterial = outlinable.FrontOutlineProperty.FillPass.Material;
                    parameters.Buffer.SetGlobalInt(FillRefHash, frontMask);
                    if (frontMaterial != null && outlinable.FrontOutlineProperty.Enabled)
                    {
                        foreach (var target in outlinable.OutlineTargets)
                        {
                            if (!target.IsVisible)
                            ...
                            parameters.Buffer.DrawRenderer(renderer, frontMaterial, target.SubmeshIndex);
                        }
                    }

                    var backMaterial = outlinable.BackOutlineProperty.FillPass.Material;
                    parameters.Buffer.SetGlobalInt(FillRefHash, backMask);
                    if (backMaterial == null || !outlinable.BackOutlineProperty.Enabled) 
                        continue;
                    ...
                    foreach (var target in outlinable.OutlineTargets)
                    {
                        ...
                        parameters.Buffer.DrawRenderer(renderer, backMaterial, target.SubmeshIndex);
                    }
                ...
                {
                    if (outlinable.DefaultOutlineProperty.FillPass.Material == null)
                        continue;

                    if (!outlinable.DefaultOutlineProperty.Enabled)
                        continue;
                    ...
                    parameters.Buffer.SetGlobalInt(FillRefHash, singleMask);
                    var fillMaterial = outlinable.DefaultOutlineProperty.FillPass.Material;
                    if (FillMaskMaterial == null)
                        continue;
                    ...
                    foreach (var target in outlinable.OutlineTargets)
                    {
                        ...
                        parameters.Buffer.DrawRenderer(renderer, fillMaterial, target.SubmeshIndex);
                    }
                }
            }
        }
