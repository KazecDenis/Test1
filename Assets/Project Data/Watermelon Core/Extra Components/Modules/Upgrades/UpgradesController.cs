    ...
    public class UpgradesController : MonoBehaviour
    {
        ...
        public void Initialise()
        {
            ...
            for (int i = 0; i < activeUpgrades.Length; i++)
            {
                ...
                var hash = string.Format(SAVE_IDENTIFIER, upgrade.UpgradeType.ToString());
//TODO:Save
      //          UpgradeSavableObject save = SaveController.GetSaveObject<UpgradeSavableObject>(hash); ;
                UpgradeSavableObject save = SaveLoad.LoadT<UpgradeSavableObject>(hash);

                upgrade.SetSave(save, hash);
