...
namespace Watermelon
{
    public static class GameLoading
    {
        ...
        public static void SetLoadingMessage(string message, string ruMessage)
        {
            loadingMessage = TextTranslator.GetText(ruMessage, message);

            float progress = 0.0f;
            if (loadingOperation != null)
                progress = loadingOperation.progress;

            OnLoading(progress, message, ruMessage);
        }
        ...
        private static IEnumerator LoadSceneCoroutine(SimpleCallback onSceneLoaded = null)
        {
            ...
            while (!loadingOperation.isDone || realtimeSinceStartup < minimumFinishTime)
            {
                ...
                OnLoading?.Invoke(1.0f, loadingMessage, loadingMessage);
            ...
            OnLoading?.Invoke(1.0f, "Done", "Успешно");
        ...
        {
            SetLoadingMessage("Loading..", "Загрузка..");
        ...
        public delegate void LoadingCallback(float state, string message, string ruMessage);
    }
}
